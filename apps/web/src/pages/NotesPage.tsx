...
export function NotesPage() {
  ...
  const selectedNote = useMemo(() => allNotes?.find(note => note.id === selectedNoteId), [allNotes, selectedNoteId]);
  ...
  const activeNotes = useMemo(() => (allNotes || []).filter(note => !note.deletedAt), [allNotes]);
  const deletedNotes = useMemo(() => (allNotes || []).filter(note => note.deletedAt).sort((a, b) => (b.deletedAt || 0) - (a.deletedAt || 0)), [allNotes]);

  const filteredNotes = useMemo(() => {
    if (!searchQuery.trim()) return activeNotes;
    const query = searchQuery.toLowerCase();
    return activeNotes.filter(note => note.title.toLowerCase().includes(query) || note.content.toLowerCase().includes(query));
  }, [activeNotes, searchQuery]);

  const sortedNotes = useMemo(() => [...filteredNotes].sort((a, b) => b.updatedAt - a.updatedAt), [filteredNotes]);

  const createNewNote = useCallback(() => {
    const newNote: Note = { id: uuidv4(), title: 'Untitled Note', content: '', createdAt: Date.now(), updatedAt: Date.now() };
  ...
  const addAttachments = useCallback((newAttachments: Attachment[]) => {
    ...
    setAllNotes(current => (current || []).map(note => note.id === selectedNoteId ? { ...note, attachments: [...(note.attachments || []), ...newAttachments], updatedAt: Date.now() } : note));
  ...
  const startResizingAttachment = useCallback(() => { isResizingAttachment.current = true; }, []);
  ...
  const resize = useCallback((e: MouseEvent) => {
    if (isResizing.current) {
      ...
      if (container) {
        ...
        if (newHeight >= 20 && newHeight <= 500) setAttachmentHeight(newHeight);
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    return () => { globalThis.removeEventListener('mousemove', resize); globalThis.removeEventListener('mouseup', stopResizing); };
  }, [resize, stopResizing]);
  ...
  return (
    <div className="flex-1 flex overflow-hidden w-full">
      <aside className="border-r border-border bg-card/30 flex flex-col relative overflow-hidden" style={{ width: `${sidebarWidth}px` }}>
        <div className="p-4 shrink-0" style={{ minWidth: 0 }}>
          <div className="relative" style={{ minWidth: 0 }}>
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none" />
            <Input placeholder="Search notes..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 w-full" style={{ minWidth: 0 }} />
          </div>
        </div>
          ...
          <div className="px-4 pb-4 space-y-2">
            {sortedNotes.length === 0 ? (
              <div className="text-center py-12 px-4">
                <p className="text-muted-foreground text-sm">{searchQuery ? 'No notes found' : 'No notes yet'}</p>
                {!searchQuery && <Button variant="ghost" size="sm" onClick={createNewNote} className="mt-2">Create your first note</Button>}
              </div>
            ) : (
              sortedNotes.map(note => (
                <NoteCard key={note.id} note={note} isActive={note.id === selectedNoteId} onClick={() => handleSelectNoteWithNavigation(note)} onDelete={(e) => { e.stopPropagation(); deleteNote(note.id); }} searchQuery={searchQuery} />
              ))
            )}
          </div>
        </ScrollArea>
      ...
      <button type="button" className="w-1 bg-border hover:bg-accent cursor-col-resize transition-colors" onMouseDown={startResizing} aria-label="Resize sidebar" />
              ...
              <Sheet open={isRecycleBinOpen} onOpenChange={setIsRecycleBinOpen}>
                <SheetTrigger asChild>
                  <Button variant="outline" size="sm" className="relative">
                    <Trash2 className="w-4 h-4 mr-1" />Recycle Bin
                    {deletedNotes.length > 0 && <Badge variant="secondary" className="ml-2 h-5 px-1.5 text-xs">{deletedNotes.length}</Badge>}
                  </Button>
                </SheetTrigger>
                <SheetContent className="w-full sm:max-w-md">
                  <SheetHeader>
                    <SheetTitle>Recycle Bin</SheetTitle>
                    <SheetDescription>Deleted notes are kept here. You can restore or permanently delete them.</SheetDescription>
                  </SheetHeader>
                  <div className="mt-6 space-y-4">
                    {deletedNotes.length > 0 && <div className="flex justify-end"><Button variant="destructive" size="sm" onClick={emptyRecycleBin}>Empty Recycle Bin</Button></div>}
                    <ScrollArea className="h-[calc(100vh-240px)]">
                      <div className="space-y-3 pr-4">
                        {deletedNotes.length === 0 ? (
                          <div className="text-center py-12"><Trash2 className="w-12 h-12 mx-auto mb-3 text-muted-foreground/50" /><p className="text-muted-foreground">No deleted notes</p></div>
                        ) : (
                          deletedNotes.map(note => (
                            <div key={note.id} className="p-4 rounded-lg border border-border bg-card">
                              <div className="flex items-start justify-between gap-3 mb-3">
                                <div className="flex-1 min-w-0">
                                  <h3 className="font-semibold text-base mb-1 truncate">{note.title || 'Untitled'}</h3>
                                  <p className="text-sm text-muted-foreground">Deleted {note.deletedAt ? new Date(note.deletedAt).toLocaleDateString() : ''}</p>
                                </div>
                              </div>
                              <div className="flex gap-2">
                                <Button variant="outline" size="sm" onClick={() => restoreNote(note.id)} className="flex-1"><RotateCcw className="w-4 h-4 mr-1" />Restore</Button>
                                <Button variant="destructive" size="sm" onClick={() => setPermanentDeleteId(note.id)} className="flex-1"><Trash2 className="w-4 h-4 mr-1" />Delete</Button>
                              </div>
                            </div>
                          ))
                        )}
        ...
        {selectedNoteId ? (
          <>
            ...
            <div className="shrink-0 flex flex-col" style={{ height: `${attachmentHeight}px` }}>
              ...
              <AttachmentManager attachments={selectedNote?.attachments || []} onAdd={addAttachments} onRemove={removeAttachment} />
            </div>
          </>
        ) : (
      ...
      <AlertDialog open={permanentDeleteId !== null} onOpenChange={() => setPermanentDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>This action cannot be undone. This will permanently delete the note from the recycle bin.</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={() => permanentDeleteId && permanentlyDeleteNote(permanentDeleteId)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">Delete Forever</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
