...
export function TodoCard({ todo, groups, onToggle, onDelete, onLinkNotes, onViewNotes, onToggleGroup, onUpdateTags, onUpdateDueDate, onUpdateTitle }: Readonly<TodoCardProps>) {
  ...
  let dueBadgeVariant: 'destructive' | 'default' | 'secondary' = 'secondary';
  if (isOverdue) {
    dueBadgeVariant = 'destructive';
  } else if (isDueSoon) {
    dueBadgeVariant = 'default';
  }
                  ...
                  {groups.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {groups.map(group => (
                        <button
                          key={group.id}
                          className="flex items-center gap-2 p-2 rounded-md hover:bg-muted cursor-pointer transition-colors w-full text-left"
